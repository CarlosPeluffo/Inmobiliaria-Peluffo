    ...
    public class RepositorioPago : Base
    {
        ...
        public int Alta(Pago p){
            ...
            using(MySqlConnection conn = new MySqlConnection(connectionString)){
                ...
                VALUES (@nro_pago, @fecha_pago, @monto, @contrato);
                SELECT last_insert_id();";
                using(MySqlCommand comm = new MySqlCommand(sql, conn)){
                    comm.CommandType = CommandType.Text;
                    comm.Parameters.AddWithValue("@nro_pago", p.NroPago);
                    comm.Parameters.AddWithValue("fecha_pago", p.FechaPago);
        ...
        public int Modificacion(Pago p){
            int res = -1;
            using(MySqlConnection conn = new MySqlConnection(connectionString)){
                string sql= @"UPDATE pagos SET nro_pago=@nro_pago,
                fecha_pago=@fecha, monto=@monto, id_contrato=@contrato
                WHERE id_pago=@id";
                using(MySqlCommand comm = new MySqlCommand(sql, conn)){
                    comm.Parameters.AddWithValue("@nro_pago", p.NroPago);
